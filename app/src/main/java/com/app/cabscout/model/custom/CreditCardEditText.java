...
public class CreditCardEditText extends AppCompatEditText {
    ...
    private final int mDefaultDrawableResId = R.drawable.ic_icon_cancel_ride; //default credit card image
    ...
    private void init() {
        if (mCCPatterns == null) {
            ...
            mCCPatterns.put(R.mipmap.card_visa, Pattern.compile("^4[0-9]{2,12}(?:[0-9]{3})?$"));
            mCCPatterns.put(R.mipmap.card_master, Pattern.compile("^5[1-5][0-9]{1,14}$"));
            mCCPatterns.put(R.mipmap.card_american, Pattern.compile("^3[47][0-9]{1,13}$"));
        }
    }
